...
cypher_query = None  # Initialize to None
database_results = None  # Initialize to None
...
if user_input:
    with st.spinner("Processing your question..."):
        ...
        try:
            ...
            cypher_query = intermediate_steps[0]["query"]  # Defined within try block
            database_results = intermediate_steps[1]["context"]  # Defined within try block
    ...
    with col2:
        if cypher_query:  # Ensure cypher_query is not None
            st.text_area("Last Cypher Query", cypher_query, key="_cypher", height=240)

    with col3:
        if database_results:  # Ensure database_results is not None
